...
MEM2WB MEM(EX2MEM ex_info) {
    ...
    if(ex_info.is_mem) {
        ...
        if(is_store) {
            ...
            uint32_t byte_off = ret.alu_out & 0b11;
            uint32_t half_off = ret.alu_out & 0b10;
            switch (mode) {
                case ACCESS_BYTE : ret.mem_we = 0b01 << byte_off; break;
                case ACCESS_HWORD: ret.mem_we = 0b11 << half_off; break;
                case ACCESS_WORD : ret.mem_we = 0b1111;           break;
                default          : ret.mem_we = 0b0;
            }
            ret.mem_waddr = ret.alu_out;
            ret.mem_wdata = ex_info.store_val;
        } else {
