...
extern int arch_on;
int ecall_checker(uint32_t cmp_n, uint32_t sc_f) { // check the testpoint
    if (cpu.gpr[10] == 0 && cmp_n > 0) {
        if (arch_on)
            printf("[%s] Test Point Pass!\n", sc_f ? "sc" : "pl");
        else
...
EX2MEM EX(ID2EX decode_info, uint32_t cmp_n, uint32_t sc_f) {
    EX2MEM ret;
    // Passthrough
    ...
    switch(decode_info.alu_op) {
        ...
        case OP_ECALL:
            ...
            ALU_CASE_ENTRY(OP_MUL, (int32_t)src1 * (int32_t)src2);
            ALU_CASE_ENTRY(OP_MULH, ((int64_t)(int32_t)src1 * (int64_t)(int32_t)src2) >> 32);
            ALU_CASE_ENTRY(OP_MULHU, ((uint64_t)src1 * (uint64_t)src2) >> 32);
            ALU_CASE_ENTRY(OP_DIV, (int32_t)src1 / (int32_t)src2);
            ALU_CASE_ENTRY(OP_DIVU, src1 / src2);
            ALU_CASE_ENTRY(OP_REM, (int32_t)src1 % (int32_t)src2);
            ALU_CASE_ENTRY(OP_REMU, src1 % src2);
